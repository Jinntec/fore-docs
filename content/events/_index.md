...
Fore is an event-driven state-engine. It reacts to events and dispatches itself a lot of events to reflect it's state.
...
|------|-------------|------------| 
| `model-construct` | is not of much pratical use but just signals that Fore is alive | `fx-model` |
...
| `ready` | is dispatched when the current Fore element is fully initialized | `fx-fore`|

For all work that should be done *before* the first rendering takes place use a 'model-construct-done' handler. 
